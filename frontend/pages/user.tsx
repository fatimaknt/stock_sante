...
export default function UserPage() {
    ...
    const load = async () => {
        try {
            setIsLoading(true);
            const data = await getJSON(API(`/users?page=${usersCurrentPage}&per_page=${usersItemsPerPage}`)) as any;
            if (data && data.items) {
                setUsers(Array.isArray(data.items) ? data.items : []);
                setUsersTotalPages(data.last_page || 1);
                setUsersTotalItems(data.total || 0);
            } else {
                        ...
                        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${usersCurrentPage === 1
                                            ...
                                            return (
                                                <React.Fragment key={page}>
                                                    ...
                                                    <button
                                                        ...
                                                        className={`min-w-[40px] px-3 py-2 rounded-lg font-medium transition-all ${usersCurrentPage === page
                                ...
                                <button
                                    ...
                                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-all ${usersCurrentPage >= Math.ceil(filteredUsers.length / usersItemsPerPage)
                                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                        : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400'
                                        }`}
                                >
